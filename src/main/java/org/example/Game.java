        ...
        for (int r = 0; r < 6; r++) {
            for (int c = 0; c <= 7 - 4; c++) {
                if (blocks[r][c].type == turn && blocks[r][c+1].type == turn &&
                        blocks[r][c+2].type == turn && blocks[r][c+3].type == turn) win();
            }
        }
        // Вертикали
        for (int c = 0; c < 7; c++) {
            for (int r = 0; r <= 6 - 4; r++) {
                if (blocks[r][c].type == turn && blocks[r+1][c].type == turn &&
                        blocks[r+2][c].type == turn && blocks[r+3][c].type == turn) win();
            }
        }
        ...
        for (int r = 0; r <= 6 - 4; r++) {
            for (int c = 0; c <= 7 - 4; c++) {
                if (blocks[r][c].type == turn && blocks[r+1][c+1].type == turn &&
                        blocks[r+2][c+2].type == turn && blocks[r+3][c+3].type == turn) win();
            }
        }
